        ...
        for key, property_value in payload.items():
            ...
            if is_dict_property_list:
                for list_value in property_value:
                    self._chupeta(
                        payload=list_value, dict_schemas=dict_schemas, parent_key=key
                    )
        return True

    def check(self, payload: dict, schema_key: str) -> bool:
        return self._chupeta(
            payload=payload, dict_schemas=schemas[schema_key], parent_key="root"
        )
