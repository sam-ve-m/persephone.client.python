...
def test_invalid_message_as_empty_byte():
    ...
    with pytest.raises(Exception, match="Given message must be dict type"):
        QueueProducerController.send_to_queue(producer=producer, topic="test", partition=0, payload=payload)
...
def test_invalid_message_as_dict_valid():
    producer.send = MagicMock(return_value=True)
    payload = {"key": "value"}
    assert QueueProducerController.send_to_queue(producer=producer, topic="test", partition=0, payload=payload) is None
